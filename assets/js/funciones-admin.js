...
function showProducts(products) {
    let productCard = products.map((product)=>
        ...
        `
            ...
            <p class="id-product col-1">${product.id}</p>
            <img class="img-product col-2" src="${product.photo}" alt="bebida de tapioca"/> 
            <p class="name-product text col">${product.name}</p>
            <p class="price-product col-2">${product.price}</p>
            <p class="description-product col-2">${product.description}</p> 
          <div class="icons col d-flex flex-column align-items-center text-center w-100 min-w-100">        
