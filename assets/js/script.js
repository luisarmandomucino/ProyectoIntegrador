function dataValidation(event) {
  ...
  const nameRegExp =
    /^[a-zA-ZÀ-ÿ\u00f1\u00d1]+(\s*[a-zA-ZÀ-ÿ\u00f1\u00d1]*)*[a-zA-ZÀ-ÿ\u00f1\u00d1]+$/;//Valida que solo se pueda escribir letras y espacios
  const emailRegexp = /^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/; //valida que el texto sea de formato email
  const numeroRegexp = /\d{10}/; //Valida un numero de 10 digitos
