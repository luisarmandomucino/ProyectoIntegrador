function dataValidation( event ){
    ...
    if(!firstName){
        ...
        message.push( "<p class='alert'>Está mal el nombre, debe ser solo letras </p>");
    }

    if(!lastName){
        message.push( "<p class='alert'>Hace falta el apellido</p>");
    }else if( !nameRegExp.test(lastName) ){
        message.push( "<p class='alert'>Está mal el apellido, debe ser solo letras </p>");
    }
    ...
    if (!email) {
        ...
        message.push("<p class='alert'>El email debe tener la forma: correo@correo.com</p>");
    }
